...
<!-- Creamos un div que es donde aparecera el resultado -->
 <div id="registro"><?php 
//Filtramos los datos para evitar que ninguno quede vacio
  if(empty($_POST['nick']) || empty($_POST["nombre"]) || empty($_POST["apellidos"]) || empty($_POST["password"]) || empty($_POST["telefono"])){
  echo "Por favor introdusca todos los datos personales";
  }else{if(isset($_POST['nick']) && isset($_POST['password']  )){
  //Generamos una variable con todos los datos recividos
  $nick= $_POST["nick"];
  $nombre= $_POST["nombre"];
  ...
  $usuario="usuario";
  $pass_cifr = password_hash($password, PASSWORD_DEFAULT, array("cost"=>15));
  try{
  //Y la introducimos en la bae de datos 
  $base=new PDO('mysql:host=localhost; dbname=cursosps', 'root', '');
  $base->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  $base->exec("SET CHARACTER SET utf8");
	$sql="INSERT INTO USUARIO VALUES (:nick ,:nombre,:apellidos,:password,:edad,:correo,:como,:telefono,:opinion,:sugerencias,:cursos,:usuario)";
  $resultado=$base->prepare($sql);
  $resultado->execute(array(":nick"=>$nick,":nombre"=>$nombre,":apellidos"=>$apellidos, ":password"=>$pass_cifr,":edad"=>$edad,":correo"=>$correo,":como"=>$como,":telefono"=>$telefono,":opinion"=>$opinion,":sugerencias"=>$sugerencias,":cursos"=>null,":usuario"=>$usuario));
  echo "Te has registrado correctamente.";
	$resultado->closeCursor();
  //En caso de que existe dara error
  }catch(Exception $e){echo "Usuario existente";}
  finally{$base=null;}
  }else{echo "No hay nada";}}?> </div>
